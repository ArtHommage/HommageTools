...
def apply_vae_encode(img_for_vae, vae, device):
    """Safely encode image with VAE handling all potential tensor format issues"""
    try:
        # STEP 1: Input verification
        logger.info("\n==== VAE ENCODE FUNCTION STARTING ====")
        logger.info(f"Input shape: {img_for_vae.shape}")
        
        # Extract dimensional info
        if len(img_for_vae.shape) == 4:
            d1, d2, d3, d4 = img_for_vae.shape
            logger.info(f"4D tensor: d1={d1}, d2={d2}, d3={d3}, d4={d4}")
            
            # Determine if already in BCHW format
            if d2 == 3 and d3 > 8 and d4 > 8:
                logger.info("Appears to be in BCHW format already")
                bchw_tensor = img_for_vae
            elif d3 == 3 and d2 > 8 and d4 > 8:
                logger.info("Appears to be in BHCW format - permuting")
                bchw_tensor = img_for_vae.permute(0, 2, 1, 3)
            elif d4 == 3 and d2 > 8 and d3 > 8:
                logger.info("Appears to be in BHWC format - permuting")
                bchw_tensor = img_for_vae.permute(0, 3, 1, 2)
            else:
                # Emergency reshape attempt
                logger.warning(f"Unusual dimensions: {img_for_vae.shape}")
                # Find channel dimension (likely the smallest)
                dims = [d2, d3, d4]
                if 3 in dims:
                    channel_idx = dims.index(3) + 1
                    logger.info(f"Found channel dim at index {channel_idx}")
                    
                    # Permute based on channel location
                    if channel_idx == 1:  # Already BCHW
                        bchw_tensor = img_for_vae
                    elif channel_idx == 2:  # BHCW
                        bchw_tensor = img_for_vae.permute(0, 2, 1, 3)
                    else:  # BHWC
                        bchw_tensor = img_for_vae.permute(0, 3, 1, 2)
                else:
                    # Force reshape - assume BHWC input
                    logger.warning("No channel dimension found - forcing reshape")
                    batch = d1
                    channels = 3
                    # Try to preserve aspect ratio in reshape
                    total_pixels = d2 * d3 * d4 / channels
                    height = int(math.sqrt(total_pixels))
                    width = int(total_pixels / height)
                    
                    logger.info(f"Reshaping to: [{batch}, {channels}, {height}, {width}]")
                    bchw_tensor = img_for_vae.reshape(batch, channels, height, width)
        else:
            logger.error(f"Unsupported tensor shape: {img_for_vae.shape}")
            raise ValueError(f"Expected 4D tensor, got {len(img_for_vae.shape)}D")
            
        # STEP 2: Verify result
        logger.info(f"Processed tensor: {bchw_tensor.shape}")
        logger.info(f"Batch={bchw_tensor.shape[0]}, Channels={bchw_tensor.shape[1]}, " +
                   f"Height={bchw_tensor.shape[2]}, Width={bchw_tensor.shape[3]}")
        
        # STEP 3: Move to VAE device
        vae_device = next(vae.first_stage_model.parameters()).device
        logger.info(f"Moving tensor to VAE device: {vae_device}")
        bchw_tensor = bchw_tensor.to(vae_device)
        
        # STEP 4: Make tensor contiguous (important for many operations)
        if not bchw_tensor.is_contiguous():
            logger.info("Making tensor contiguous")
            bchw_tensor = bchw_tensor.contiguous()
        
        # STEP 5: VAE Encode
        logger.info(f"Final tensor shape before encoding: {bchw_tensor.shape}")
        logger.info(f"Value range: min={bchw_tensor.min().item():.4f}, max={bchw_tensor.max().item():.4f}")
        
        # Ensure we pass correct tensor order to encoder
        """if bchw_tensor.shape[1] != 3:
            logger.error(f"ERROR: Expected 3 channels, got {bchw_tensor.shape[1]}")
            # Last-resort attempt - force channel dimension to be correct
            bchw_tensor = bchw_tensor.reshape(bchw_tensor.shape[0], 3, -1, bchw_tensor.shape[3])
            logger.info(f"Emergency reshape: {bchw_tensor.shape}")"""
        
        # Call VAE encode directly
        logger.info(f"FINAL Final tensor shape before encoding: {bchw_tensor.shape}")
        samples = vae.encode(bchw_tensor)
        return samples

    except Exception as e:
        logger.error(f"VAE encoding error: {e}")
        raise
...
def apply_ksampler(
    ...
    """Apply KSampler to latent with conditioning."""
    try:
        # Determine device from model parameters (or fallback to CPU)
        try:
        ...
        # Generate random noise based on seed
        noise = comfy.sample.prepare_noise(latent, seed, device)
        
        # Perform sampling
        logger.info(f"Starting sampling with CFG={cfg}, denoise={denoise}")
        ...
        logger.error(f"Error in apply_ksampler: {e}")
        import traceback
        logger.error(traceback.format_exc())
        # Return original latent if sampling fails
        return latent
