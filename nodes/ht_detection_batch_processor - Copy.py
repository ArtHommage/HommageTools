...
def ensure_bchw_format(tensor, name="Unknown"):
    ...
    # Verify dimensions are reasonable for BCHW format
    if not (result.shape[1] <= 4 or (result.shape[2] > 0 and result.shape[3] > 0)):
...
def _direct_vae_encode(img, vae):
    """Directly encode image with VAE, circumventing dimension issues."""
    # Get the height and width of the image
    b, h, w, c = img.shape
    
    # First convert to expected NCHW
    x = img.permute(0, 3, 1, 2).contiguous()
    
    # Generate a view of the tensor with the dimensions explicitly correct
    # to avoid dimension swapping inside VAE
    x_view = x.view(b, c, h, w)
    
    # Try to encode with the view
    try:
        encoder = vae.first_stage_model.encoder
        with torch.no_grad():
            z = encoder(x_view)
        samples = vae.get_first_stage_encoding(z)
        return samples
    except Exception as e:
        logger.error(f"Direct VAE encode failed: {e}")
        # Try fallback without view if the above fails
        return vae.encode(x)
...
def apply_ksampler(
    ...
    try:
        ...
        logger.info(f"Creating KSampler with {steps} steps, device={device}, sampler={sampler_name}, scheduler={scheduler}")
...
class HTDetectionBatchProcessor:
    ...
    FUNCTION = "process"
    RETURN_TYPES = ("IMAGE", "MASK", "SEGS")
    RETURN_NAMES = ("processed_images", "processed_masks", "segs") 
    ...
    def process(
        ...
        negative=None,
        labels="all"
    ) -> Tuple[List[torch.Tensor], List[torch.Tensor], Any]:
        """
        ...
        if not segs[1] or len(segs[1]) == 0:
            ...
            # Return properly formatted empty structure
            return [], [], ((height, width), [])
        ...
        for i, seg in enumerate(segs[1]):
            ...
            if use_ksampler and denoise > 0:
                try:
                    ...
                    # First verify the image has valid dimensions
                    if 0 in cropped_image.shape[1:3] or cropped_image.shape[1] < 16 or cropped_image.shape[2] < 16:
                        raise ValueError(f"Image too small for VAE: {cropped_image.shape}")
                        
                    # Validate cropped image format and fix if needed
                    cropped_image = ensure_bhwc_format(cropped_image, "pre_vae_image")
                        
                    # Make sure dimensions are at least 32x32 and divisible by 8
                    vae_h = max(32, get_nearest_divisible_size(cropped_image.shape[1], 8))
                    vae_w = max(32, get_nearest_divisible_size(cropped_image.shape[2], 8))
                        
                    # Resize for VAE compatibility if needed
                    if vae_h != cropped_image.shape[1] or vae_w != cropped_image.shape[2]:
                        logger.info(f"Resizing for VAE: {cropped_image.shape[1]}x{cropped_image.shape[2]} -> {vae_h}x{vae_w}")
                        
                        # Convert to BCHW for resize
                        img_bchw = ensure_bchw_format(cropped_image, "vae_resize_input")
                        
                        # Check if dimensions are valid for interpolation
                        if 0 in img_bchw.shape[2:]:
                            raise ValueError(f"Invalid dimensions for interpolation: {img_bchw.shape}")
                        ...
                        # Convert back to BHWC
                        img_for_vae = ensure_bhwc_format(resized_bchw, "vae_resize_output")
                    else:
                        img_for_vae = cropped_image
                        
                    # Using direct VAE encoding
                    logger.info("Using direct VAE encoding helper...")
                    b, h, w, c = img_for_vae.shape
                    logger.info(f"VAE input dimensions: batch={b}, height={h}, width={w}, channels={c}")

                    # Directly encode with helper
                    vae_samples = _direct_vae_encode(img_for_vae, vae)
                    logger.info(f"VAE encoded shape: {vae_samples.shape}")
        ...
        return processed_images, processed_masks, segs
